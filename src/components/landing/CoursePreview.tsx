...
const CoursePreview = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [chaptersRes, modulesRes] = await Promise.all([supabase.from('chapters').select('id, title, description, order_number').eq('status', 'published').order('order_number', {
        ascending: true
      }), supabase.from('modules').select('id, title, description, chapter_id, order_number').eq('status', 'published').order('order_number', {
        ascending: true
      })]);
  ...
  return <section className="bg-muted/50 py-12 sm:py-16 md:py-20 lg:py-28">
      <div className="container px-4 sm:px-6">
        <div className="mx-auto mb-8 sm:mb-12 max-w-2xl text-center">
          ...
          <p className="text-base sm:text-lg text-muted-foreground">
            {totalModules > 0 ? `${totalModules} video lessons with practical exercises you can use today` : "Comprehensive video lessons with practical exercises you can use today"}
          </p>
        </div>
        ...
        <div className="mx-auto max-w-3xl">
          {loading ? <div className="flex justify-center py-12">
            ...
            </div> : chaptersWithModules.length === 0 ? <div className="text-center py-12 text-muted-foreground">21</div> : <div className="space-y-3 sm:space-y-4">
              {chaptersWithModules.map((chapter, chapterIndex) => {
            const isExpanded = expandedChapter === chapter.id;
            const teasers = chapterTeasers[chapterIndex + 1] || [];
            return <div key={chapter.id} className="rounded-xl border border-border bg-card overflow-hidden transition-all duration-300 hover:shadow-card">
                    {/* Chapter header - clickable */}
                    <button onClick={() => toggleExpanded(chapter.id)} className="w-full p-4 sm:p-5 bg-card hover:bg-muted/30 transition-colors text-left">
                      <div className="flex items-start gap-3 sm:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {teasers.length > 0 && <div className="mt-3 flex flex-wrap gap-2">
                              {teasers.map((teaser, i) => <span key={i} className="inline-flex items-center gap-1 text-xs bg-accent/10 text-accent-foreground px-2 py-1 rounded-full">
                    ...
                    {isExpanded && chapter.modules.length > 0 && <div className="border-t border-border bg-muted/20">
                        {chapter.modules.map((module, moduleIndex) => <div key={module.id} className="flex items-center gap-3 px-4 sm:px-5 py-3 border-b border-border/50 last:border-b-0">
